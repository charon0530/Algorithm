function solution(n, paths, gates, summits) {
    function BFS(startNode, summitNode) {
        ...
        const queue = [];
        queue.push([startNode, 0, new Set([startNode])]);


        while (queue.length) {
            const [curNode, curMaxIntensity, checkedSet] = queue.shift();
            if (curNode === summitNode) {
                console.log("start : ", startNode, " end : ", summitNode, curMaxIntensity)
                minValue = Math.min(minValue, curMaxIntensity)
                continue;
            }
            if (curMaxIntensity >= minValue) continue;
            ...
            for (const [next, cost] of graph[curNode]) {
                ...
                queue.push([next, Math.max(curMaxIntensity, cost), nextCheckSet])
            }
        }
        return minValue
    }
