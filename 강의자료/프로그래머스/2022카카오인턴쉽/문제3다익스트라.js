...
function solution(alp, cop, problems) {
    ...
    const pq = new PQ();
    const dist = Array.from({ length: Math.max(targetAlp + 1, alp + 1) }, () => new Array(Math.max(targetCop + 1, cop + 1)).fill(Number.MAX_SAFE_INTEGER))
    const visited = Array.from({ length: Math.max(targetAlp + 1, alp + 1) }, () => new Array(Math.max(targetCop + 1, cop + 1)).fill(0))
    pq.push([alp, cop, 0]);
    dist[alp][cop] = 0;
    ...
    while (pq.size() > 0) {
        ...
        for (const [reqAlp, reqCop, rwdAlp, rwdCop, dur] of problems) {
            ...
            if (curAlp >= reqAlp && curCop >= reqCop) {
                const newAlp = Math.min(curAlp + rwdAlp, targetAlp);
                const newCop = Math.min(curCop + rwdCop, targetCop);
                if (dist[newAlp][newCop] <= curTime + dur) continue;

                dist[newAlp][newCop] = curTime + dur
                pq.push([newAlp, newCop, curTime + dur]);
            }
        }
    }
}
